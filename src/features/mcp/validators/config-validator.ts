import { MCP_AUTH_MODES } from "../constants";
import type {
  InstalledMCPServer,
  MCPCategory,
  MCPCapability,
...
/**
 * Validation report across a collection of installed servers.
 * Includes both per-server issues and cross-server issues
 * (duplicate names, etc.).
 */
export interface ValidationReport {
  servers: ValidationResult[];
  /** Issues spanning multiple servers. */
  globalIssues: ValidationIssue[];
  /** Aggregate ok = no error-severity issues anywhere. */
  ok: boolean;
  /** Summary counts. */
...
export function validateServer(
  ...
  if (server.transport === "stdio") {
    ...
    // Heuristic: a bare executable that a platform shell would typically resolve
    // without an exec extension. We don't perform an actual `which`, but we
    // warn when the executable is a path without an obvious base.
    if (server.command && /^\s*$/.test(server.command)) {
      issues.push({
        severity: "error",
        code: "MISSING_EXECUTABLE",
        message: "Command is empty; cannot spawn the server.",
        field: "command",
      });
    }
