...
const variantConfigs: Record<EmptyStateVariant, { icon: React.ReactNode; title: string; description: string }> = {
  default: {
...
export function EmptyState({
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center py-16 px-4 text-center", className)}>
      <div className={cn("mb-4 text-[var(--color-text-muted)]", variant === "loading" && "text-[var(--color-accent)]")}>
        {icon || config.icon}
      </div>
