...
/**
 * Command Palette (âŒ˜K) â€” module switcher + quick actions.
...
export function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const items = React.useMemo((): CommandItem[] => {
    const moduleItems: CommandItem[] = MODULES_ORDERED.map((m) => ({
      ...
      icon: m.icon,
      shortcut: m.id === "dashboard" ? "âŒ˜1" : undefined,
      action: () => {
        navigate(m.id);
        onClose();
      },
            ...
            <div className="overflow-hidden rounded-xl border border-border-default bg-bg-elevated/95 shadow-2xl backdrop-blur-xl">
              ...
              <div className="border-t border-border-subtle px-3 py-2">
                <p className="flex items-center justify-center gap-2 text-[10px] text-fg-subtle">
                  <kbd className="rounded border border-border-strong bg-white/5 px-1.5">â†‘</kbd>
                  <kbd className="rounded border border-border-strong bg-white/5 px-1.5">â†“</kbd>
                  navigate Â· <kbd className="rounded border border-border-strong bg-white/5 px-1.5">âŽ</kbd> select Â· <kbd className="rounded border border-border-strong bg-white/5 px-1.5">Esc</kbd> close
                </p>
              </div>
            </div>
          </motion.div>
