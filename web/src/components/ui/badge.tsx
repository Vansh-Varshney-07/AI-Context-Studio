  ...
  "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors whitespace-nowrap",
  {
    variants: {
      variant: {
        ...
        outline: "bg-transparent border border-[var(--color-border-strong)] text-[var(--color-text-secondary)]",
        dot: "relative pl-6",
      },
    },
  ...
  return (
    <span className={cn(badgeVariants({ variant }), className)} {...props}>
      {isDot && dotColor && (
        <span
          className={cn(
            "absolute left-2 top-1/2 -translate-y-1/2 size-1.5 rounded-full",
            {
