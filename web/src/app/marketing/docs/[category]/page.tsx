  ...
  if (!cat) {
    return (
      <div className="container-app py-16 text-center">
        <h1 className="text-3xl font-bold text-[var(--color-text-primary)] mb-4">Category not found</h1>
        <p className="text-[var(--color-text-secondary)]">The documentation category you're looking for doesn't exist.</p>
    ...
    <div className="container-app py-16 space-y-8">
      ...
      <div className="space-y-4">
        {cat.items.map((item, index) => (
          <div
            key={item.href}
            className="group p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-surface)] hover:border-[var(--color-border-strong)] transition-colors"
          >
            <Link
              href={item.href}
              className="flex items-center gap-3 text-[var(--color-text-primary)] hover:text-[var(--color-accent)] transition-colors"
            >
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-accent-light)] text-[var(--color-accent)] group-hover:scale-110 transition-transform duration-200">
            ...
            <p className="mt-2 ml-11 text-sm text-[var(--color-text-muted)] max-w-2xl">
