 ...
 * to this union + a definition in `fields.ts`. Pure data â€” never a UI edit.
 */
...
export interface EngineField {
  ...
  placeholder?: string;
  /** For select / multiselect â€” uses shared vocab ids. */
 ...
 *
 * Each builder MUST declare which EngineFieldIds it consumes â€” this powers
...
export interface EngineBlueprint {
  ...
  label: string;
  description: string;
  /** Filename hint for export â€” `${slug}.${ext}`. */
  filenameHint: string;
...
export interface EngineOutput {
  ...
  filename: string;
  content: string;
  /** Field ids the producing blueprint declared â€” for the schema view. */
