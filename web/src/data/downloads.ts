...
export const downloads: PlatformDownload[] = [
  {
    ...
    verifySignature: "Windows installer is code-signed with EV certificate. Verify in file Properties → Digital Signatures.",
  },
  {
    os: "macOS",
    icon: "Monitor",
    ...
    verifySignature: "macOS builds are notarized by Apple. Verify with `spctl -a -v /Applications/AI\\ Context\\ Studio.app`",
  },
  {
    os: "Linux",
    icon: "Monitor",
    ...
    verifySignature: "Linux binaries are signed with GPG. Verify with `gpg --verify *.sig *.AppImage`",
  },
];
